...
export class BookmarkTableComponent implements OnInit {
  ...
  bookmarks: Bookmark[] = [];
  dataSource: MatTableDataSource<Bookmark> = new MatTableDataSource<Bookmark>(this.bookmarks);
  ...
  ngOnInit(): void {
    ...
    .pipe(
      tap( (param: string) => this.dataSource.filter = param )
    ).subscribe();
  }

  getBookmarksGroups() {
    return this.bookmarks ? [...new Set(this.bookmarks.map(bm => bm.group))] : [];
  }
